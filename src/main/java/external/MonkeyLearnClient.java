...
//	}
	...
	public static List<List<String>> extractKeywords(String[] text) {
		...
		try {
			response = ml.extractors.extract(MODEL, text, extraParams);//API to get keyword from description
	...
	//conversion from JSONArray to list<list<string>>
	private static List<List<String>> getKeywords(JSONArray mlResultArray){
		List<List<String>> topKeywords = new ArrayList<>();
		
		for (int i = 0; i < mlResultArray.size(); i++) {
			List<String> keywords = new ArrayList<>();//output result
			JSONArray keywordsArray = (JSONArray) mlResultArray.get(i); //cast: input may have other type
